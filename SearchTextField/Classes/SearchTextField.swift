...
@IBDesignable
open class SearchTextField<T>: UITextField, UITableViewDelegate, UITableViewDataSource {
    ...
    open var itemSelectionHandler: SearchTextFieldItemHandler<T>?
    ...
    fileprivate let cellIdentifier = "APSearchTextFieldCell"
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var reusableCell: UITableViewCell? = tableView.dequeueReusableCell(withIdentifier: cellIdentifier)
      
        if reusableCell == nil {
            reusableCell = UITableViewCell(style: .subtitle, reuseIdentifier: cellIdentifier)
        }

        guard let cell: UITableViewCell = reusableCell else {
          fatalError()
        }
        ...
        cell.textLabel?.attributedText = result.attributedTitle
        cell.detailTextLabel?.attributedText = result.attributedSubtitle
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let itemSelectionHandler = self.itemSelectionHandler else {
            self.text = filteredResults[indexPath.row].title
            return
        }

        itemSelectionHandler(filteredResults, indexPath.row)
...
public struct SearchTextFieldTheme {
    ...
    public init(cellHeight: CGFloat, bgColor: UIColor, borderColor: UIColor, separatorColor: UIColor, font: UIFont, fontColor: UIColor, subtitleFontColor: UIColor? = nil) {
...
open class SearchTextFieldItem<T> {
    // Private vars
...
public typealias SearchTextFieldItemHandler<T> = (_ filteredResults: [SearchTextFieldItem<T>], _ index: Int) -> Void
